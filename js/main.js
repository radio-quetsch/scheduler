    ...
    Object.keys(myJson).forEach(function(item) {
        ...
        if (item != "AIRTIME_API_VERSION") {
            ...
            if (now.getMonth() == dayDate.getMonth() && now.getDate() == dayDate.getDate()) {
                weeklySchedulerTab += '<li class="nav-item bg-secondary">' + '<a class="nav-link text-white active today" id="' + item + '-tab" data-toggle="tab" href="#' + item + ' " role="tab" aria-controls="' + item + '" aria-selected="true">' +  weekday  + " " + thisDayDate + '</a>' + '</li>';
